...
import {Table, Nav, NavItem, NavLink, Button, Row, Col, Input} from "reactstrap";
...
class GetDoctor extends React.Component {
	...
	componentDidMount() {
		...
		axios.get("http://localhost:4000/doctor/doctorList", { headers: headers })
			.then((resp) => {
				console.log("hi");
				console.log(resp);
	...
	handleDelete(id) {
		console.log(id);
		axios.delete(`http://localhost:4000/doctor/deleteDoctor/${id}`)
			.then((res) => {
				alert(res.data);
				...
				<Row>
					...
					<Col className="mt-3">
						...
						<Table
							...
							<tbody>
								{typeof this.state.doctors != undefined ? (this.state.doctors.filter((doctor, index) => {
