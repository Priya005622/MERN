...
import {Col, Nav, NavItem, NavLink, Row, Table, Input, Button} from "reactstrap";
...
class GetAppointments extends React.Component {
	...
	componentDidMount() {
		...
		axios.get(`http://localhost:4000/appointment/AppointmentList/${id}`,
				{ headers: headers }
			).then((res) => {
				console.log(res);
				try {
	...
	handlePres(appointment) {
		...
		axios.post("http://localhost:4000/appointment/addPrescription",
				{
				...
				<Row>
					...
					<Col className="mt-3">
						...
						<Table
							...
							{typeof this.state.appointments != undefined ? (this.state.appointments.filter((appointment) => {
											...
											<tr>
												...
												{this.state.isOpen ? (
													<td style={{columnWidth:"200px",}}>
														<Input type="text" onChange={(e) =>this.setState({prescription:e.target.value,})}/>
														<Button style={{backgroundColor:"green",}}
															className="mt-2"
															onClick={() =>this.handlePres(appointment)}>
															Add
														</Button>
													</td>
												) : (
